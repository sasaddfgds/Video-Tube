...
if ($video_id > 0) {
    try {
        $stmtCheck = $db->prepare("SELECT 1 FROM dislikes WHERE video_id = ? AND user_id = ?");
        $stmtCheck->execute([$video_id, $user_id]);
        $alreadyDisliked = (bool)$stmtCheck->fetchColumn();
        ...
        if ($alreadyDisliked) {
            ...
            $db->prepare("INSERT INTO dislikes (video_id, user_id) VALUES (?, ?)")->execute([$video_id, $user_id]);
            $action = 'disliked';
        }
